...
const SoilHumidity = (props) => {
    ...
    function getValues(data, select) {
        ...
        for (var i = 0; i < data.length; i++) {
            var e = data[i];
            if (
                e.date.getTime() <= now.getTime() - count * step &&
                e.date.getTime() > now.getTime() - (count + 1) * step
            ) {
                var newTime = new Date(now.getTime() - count * step);
                res.push({ value: e.value, time: getTime(newTime) });
                count++;
                if (count == 10) return res;
            } else if (e.date.getTime() < now.getTime() - (count + 1) * step) {
                while (e.date.getTime() < now.getTime() - (count + 1) * step) {
                    var newTime = new Date(now.getTime() - count * step);
                ...
                newTime = new Date(now.getTime() - count * step);
                res.push({ value: e.value, time: getTime(newTime) });
                count++;
                if (count == 10) return res;
            }
